...
## Generation 15-R1 Windows compatibility authorization

The product owner explicitly authorized a corrected Generation 15 chain after
the earlier local authorization `feafbbb094e9bc854dbf632b2e06fc1a7346db9f`
and start `01993856e740e894f56601660b412eac87e991e3` were found non-canonical:
their topology was correct, but the task header still described Generation 14
as blocked. Those local objects remain immutable history and are not candidate,
delivery, review, CI, merge or closure evidence.

This corrected authorization has exact ordered parents
`[94c87f28436e2ea8899c9a407e1f1413de893603,
1b8828dc31907d7daaeabe767ff400c9dc020a2e]`. It may repair only the three
Windows test-infrastructure classes proven by exact-head run `33839681522`:
temporary-repository CRLF/exact-HEAD/immutable digest drift, the Chinese-diff
regression's incomplete environment through the Job Object path, and the
KeyboardInterrupt fake process without a Windows `_handle`.

Engineering changes are limited to `tests/test_g0_project_status.py` and
`tests/test_verify_full_ci.py`. Validator/verifier production code, complete
test collection and assertions, 870-second and hosted 15/20-minute limits,
...
### Generation 14 sole complete local validation

Exact accepted integration subject
`5d4fa473bf3ca25ee7483d86461f49f5f3b068eb` consumed Generation 14's one
...
platform-only skips in 276.45 seconds, Python dependency integrity, offline
...
The strict child is only the new frozen local `awaiting_review` candidate.
