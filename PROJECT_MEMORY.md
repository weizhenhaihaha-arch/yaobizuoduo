...
- Offline application boundaries and the approved deterministic frontend exist through M6-T01. The M6-T02 signal detail, history, observation-statistics, and help implementation plus its frontend/backend timeline-event contract repair are present and awaiting autonomous review. No live exchange transport or trading execution is implemented.
...
- M0 through M5 and M6-T01 are complete and approved; M6-T02 is `awaiting_review` after aligning its timeline event fixture, type, rendering, and regression tests with the approved `api.v1` detail payload.
